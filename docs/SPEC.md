...
This document specifies behavior. `unityyamlmerge_fix.py` is the executable
reference for sections 2 and 3 and for the checker side of section 4: port
its functions, do not redesign them. Where prose and reference disagree, the
reference wins; report the discrepancy in the PR instead of silently picking.
...
## 2. Codec (reference: reemit_plain .. reserialize)

Port these functions 1:1, same names snake_case:
...
Bug-compatibility: when the closing quote is missing, the reference's
`rfind` returning -1 silently drops the last character (`CutAtLastQuote`
in the C# port). Preserve exactly.
...
## 3. Model (reference: table_entries, refid_records, DOC_ANCHOR)
