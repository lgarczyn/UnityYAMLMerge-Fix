//! Plain 3-way line merge, std only. SPEC section 4.6. Packet P5.
//! Two-way LCS diff composed into 3-way hunks; conflict hunks labeled
//! ours/base/theirs. Fixture parity with git merge-file.
//!
//! Lines are `&str` slices, each carrying its own terminator, so output
//! is byte-reproducible against git. Matching lines are compared by exact
//! bytes. The engine is textual only; it knows nothing about YAML.
...
/// A change region of a two-way diff, as a base line range [start, start+len).
struct Change {
    o_start: usize,
    o_len: usize,
}
...
/// Diff base against one side. Returns the change regions in base order plus
/// a per-base array `match_side[k]` holding the aligned side index when base
/// line k is matched, else `UNMATCHED`.
...
/// Common prefix and suffix are matched greedily first, which shrinks the DP
/// and keeps alignment stable at the edges. This is a longest common
/// subsequence, not git's Myers diff, so on highly repetitive input the two
/// may pick different equally-long alignments. Parity with git is asserted
/// only on the committed realistic fixtures, where lines are near-unique.
fn lcs_pairs(o: &[&str], s: &[&str]) -> Vec<(usize, usize)> {
    ...
    if n > 0 && m > 0 {
        ...
        // Backtrack from the front, taking a match whenever it stays optimal
        // and otherwise advancing base first, so ties resolve consistently.
        let mut i = 0;
        let mut j = 0;
        while i < n && j < m {
...
/// The boundaries are LCS-matched by construction, so the sentinel branch
/// never applies to an unmatched line.
fn side_range(
    match_side: &[usize],
    side_len: usize,
    r_start: usize,
    ...
    let start = if r_start == 0 {
        ...
        // match_side[r_start - 1] is a matched boundary line by construction.
...
/// Compose a plain 3-way line merge. `base`, `ours`, `theirs` are line
/// slices. Regions are returned in output order; conflicts carry all three
/// sides untrimmed.
pub fn diff3<'a>(
    base: &'a [&'a str],
    ours: &'a [&'a str],
    ...
    // side 0 = ours, side 1 = theirs. Sort by base start, ours before theirs.
...
/// Render as two-way conflict markers, ours/theirs, no base section.
...
/// Render as three-way (diff3) conflict markers, showing the base section
/// between ours and theirs, untrimmed, matching `git merge-file --diff3`.
