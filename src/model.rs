//! Parsers for documents, table entries, and RefIds records, with line
//! spans for reassembly. SPEC section 3. Packet P4.
//! Reference functions: table_entries, refid_records, DOC_ANCHOR.
//!
//! These mirror the Python reference one to one. Content extraction matches
//! table_entries and refid_records exactly; the added value is line spans
//! for reassembly and explicit duplicate-key tracking. Parsers work on
//! unwrapped text split on '\n'; a trailing '\r' stays in the line content,
//! as in the reference. CRLF normalization is the caller's job.
...
/// One `m_TableData` entry, keyed by `m_Id`. `localized` is the joined
/// `m_Localized` text, `rids` the `m_Metadata` rid set, `loc_count` the
/// number of `m_Localized` fields seen. `spans` holds one range per physical
/// occurrence; duplicates accumulate content but keep every occurrence span.
...
/// Result of [`table_entries`]: entries keyed by id, ids in first-seen order,
/// and the set of ids that occur more than once.
#[derive(Clone, Debug, PartialEq, Eq)]
...
/// One `references`/`RefIds` record, keyed by rid. `payload` is the joined
/// record lines minus `- id:` items, `ids` the `- id:` value set. `spans`
/// holds one range per physical occurrence.
#[derive(Clone, Debug, PartialEq, Eq)]
...
/// Result of [`refid_records`]: records keyed by rid, rids in first-seen
/// order, and the set of rids that occur more than once.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct RefData {
...
/// span, repeated anchors, and the synthetic preamble span before the first
/// document marker.
#[derive(Clone, Debug, PartialEq, Eq)]
...
// Take one or more ASCII digits from the front of `s`. Reference `\d+`.
...
// Take an optional '-' then one or more ASCII digits. Reference `-?\d+`.
...
// Section boundary rule: exactly two leading spaces, not `  -`, and the third
// character exists and is not whitespace. Reference:
// `line.startswith("  ") and not line.startswith("  -") and line[2:3].strip()`
fn is_section_key(line: &str) -> bool {
...
/// Split whole documents by `&anchor` per [`Documents`]. Ported from the
/// reference `DOC_ANCHOR` regex, extended with spans and duplicate tracking.
...
/// Parse `m_TableData` entries per [`TableData`]. Mirrors the reference
/// `table_entries`: duplicate ids accumulate into the first record, and
/// content rules are the caller's concern.
...
/// Parse `references`/`RefIds` records per [`RefData`]. Mirrors the reference
/// `refid_records`: `- id:` items feed the id set, everything else the
/// payload, and duplicate rids accumulate into the first record.
