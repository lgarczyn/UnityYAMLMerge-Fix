//! Post-merge self-check: the reference validate_merge as a checker over
//! the final output. A failure here is a bug in merge, not a conflict.
//! SPEC sections 4 and 5.1. Packet P8.
//!
//! Ported function by function from the reference validate_merge and its
//! helpers _value_rule, _set_rule and _presence_rule. The reference builds
//! its keyed views with table_entries, refid_records and DOC_ANCHOR; here
//! those come from the P4 model parsers, projected down to the same tuples
//! the reference compares: entries to (text, rids, loc_count), records to
//! (payload, ids), documents to the anchor set. Line spans that the model
//! tracks for reassembly are dropped, since the checker only compares
//! content, exactly as the reference does.
//!
//! Inputs are CRLF-normalized before parsing, mirroring the reference's
//! leading `x.replace("\r\n", "\n")`. Inherited duplicate keys carry the
//! reference skip rules: keys already duplicated in an input are checked for
//! presence only, never for content.
...
// m_Metadata rid set and the m_Localized field count. Equality here matches
// the reference `side[key] == b[key]` tuple compare.
...
// Scalar 3-way: a silent side-pick or an invented value is as much a loss as
// a drop. `b` is None when the key is absent from base.
...
// honored. `b` None means the empty set.
fn set_rule(
    what: &str,
...
// Presence per SPEC 4.1. `verify_both` runs when the key is on both sides and
// survived; `verify_copy` when it was added on one side and survived, with
// that side's value. Both receive `viols` to append to.
...
/// `base`, `ours` and `theirs`. An empty result means verified. Argument
/// order matches the reference validate_merge.
pub fn validate_merge(base: &str, ours: &str, theirs: &str, merged: &str) -> Vec<String> {
    ...
    // Documents, keyed by &anchor. Only drops are checked, matching the
    // reference: presence of a document body is verified structurally below
    // through its records, not line by line here.
...
mod tests {
    ...
    // Shape builders mirror tests/conftest.py so the ported scenarios read the
    // same as the reference suite.
    ...
    // Each scenario feeds a fabricated native-tool corruption straight to the
    // verifier. All must be caught: validate_merge returns a non-empty list.
