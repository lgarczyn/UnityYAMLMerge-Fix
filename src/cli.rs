//! CLI driver contract. SPEC section 5. Packet P9.
//!
//! Two entry shapes. The driver `uymerge BASE REMOTE LOCAL OUTPUT` runs the
//! full pipeline: read, unwrap, merge_file, rewrap, self-check with
//! validate_merge, restore CRLF, write OUTPUT. The batch test modes
//! `--batch-reserialize` and `--batch-unwrap` mirror the reference harness in
//! oracle/py_batch.py so oracle/differential.sh can byte-compare us to it.
//!
//! Exit contract, SPEC 5.1-5.3: exit 0 only on a conflict-free merge that also
//! passes the self-check; any conflict or any self-check failure exits 1 with a
//! marked-up OUTPUT. A driver that fails silently is how localization work got
//! reverted before, so a bug must never leave OUTPUT looking like clean ours.
//! Release builds unwind rather than abort and the driver catches any panic,
//! turning it into the same whole-file conflict.
...
// "unwrap" width: large enough never to fold. Mirrors the reference INF.
const INF: usize = 1_000_000_000;

const USAGE_RC: u8 = 2;

/// Dispatch on argv. Batch modes when arg 1 names one, else the merge driver.
...
// The clean/conflict/self-check outcome of the merge pipeline. Conflict carries
// the marked-up output; VerifyFailed carries the violations to report.
enum MergeOutcome {
    Clean(String),
    Conflict(String),
...
fn driver(args: &[String]) -> ExitCode {
    ...
    // CRLF is decided from the original ours, SPEC 5.4. Lossy decode is fine:
    // it never alters the ASCII terminators the count looks at.
...
// The merge pipeline on LF-normalized inputs. Unwrap all three, merge by
// document and record, rewrap, then self-check the rewrapped result against the
// unwrapped inputs, SPEC 5.1. A clean merge that fails the check is a bug and
// resolves to VerifyFailed, never to a silent exit 0.
...
// Batch test mode. One output file per non-blank list entry, named by the
// entry's index, LF-preserving. A decode failure writes an empty `<i>.error`
// instead. Mirrors oracle/py_batch.py exactly for the differential oracle.
...
// Read a file, reporting the path on failure so a missing input is diagnosable.
...
// A whole-file ours/theirs conflict, SPEC 5.3. Ported from the reference
// conflict_file. The output must be unmistakable: git leaves the driver's
// OUTPUT as the working-tree file, so a marker-less file reads as resolved.
...
mod tests {
    ...
    // different documents, one added by each side. The per-document keyed merge
    // keeps both cleanly, but the file-wide verifier sees a duplicated entry.
