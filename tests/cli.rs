//! Integration tests driving the built uymerge binary. SPEC section 5.
//! Covers a clean merge, a conflict, the self-check failure path, both batch
//! modes and CRLF restore.
...
// Run the driver over three input texts. Returns the exit code and OUTPUT bytes.
// REMOTE is theirs, LOCAL is ours, matching the git merge-driver order.
...
fn self_check_failure_leaves_whole_file_conflict() {
    ...
    // The same m_Id in two documents, one added on each side. The keyed merge
    // keeps both cleanly but the file-wide self-check rejects the duplicate, so
    // the driver must emit a whole-file conflict, SPEC 5.3, never a clean exit.
    let base = "%YAML 1.1\n--- !u!1 &1\nGameObject:\n  m_Name: root\n";
...
// Build a two-entry list file: one valid UTF-8 input and one invalid, so we
// exercise both the reserialize output and the .error path.
fn batch_list(dir: &Path) -> (PathBuf, String) {
    let good = dir.join("good.asset");
    // a long plain scalar that folds under the reserialize width but stays on
    // one line when unwrapped
