...
//!
//! Fixtures under tests/fixtures/diff3 are generated by
//! oracle/gen_diff3_cases.sh, whose expected outputs come from git itself.
...
fn expected_rc(path: &Path) -> bool {
    // git merge-file exits with the conflict count; any nonzero means a
    // conflict was emitted, 0 means a clean merge.
    read(path).trim() != "0"
}
...
fn check_case(dir: &Path) {
    ...
    // Two-way style: compare only where git did not zealously fuse conflicts
    // that diff3 keeps apart. The generator drops the marker file when it did.
    ...
    if dir.join("expected.merge.parity").exists() {
        ...
        // uymerge keeps the smaller diff3-hunk conflicts; it still must flag
        // a conflict exactly when git's two-way merge did.
        assert_eq!(
            merge_conflict,
            expected_rc(&dir.join("expected.merge.rc")),
