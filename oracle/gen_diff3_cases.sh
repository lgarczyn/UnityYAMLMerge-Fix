...
#
# For each named case, write base/ours/theirs and capture git merge-file
# output and exit status in both the two-way and diff3 marker styles, using
# the labels ours/base/theirs. The Rust parity test replays these committed
# fixtures, so the expected bytes come from git itself, not by hand.
#
# The diff3 style is the authoritative oracle: uymerge matches it byte for
# byte, including the base section that carries the ours/base/theirs labels.
# git's default two-way style additionally applies a zealous heuristic that
# fuses conflicts separated by three or fewer common lines. SPEC 5.2 wants
# uymerge's markers at the smallest diff3-hunk unit, so uymerge does not copy
# that fusion. When git fuses, we drop an expected.merge.parity marker and
# the test skips the two-way byte comparison for that case; diff3 parity
# still covers it fully.
#
...
# Both delete the same middle line, one also edits nearby: clean delete plus edit.
