#!/bin/sh
# Reproducible timing for a no-op merge of a large corpus file. The no-op
# path base == ours == theirs drives the full codec unwrap and rewrap plus
# the model parse with no conflicts, which is the P11 budget case. Reports
# the best wall time over several runs, since the minimum is the least
# noisy estimate of the code cost. Hightower only; needs a checkout.
#
